...
impl ManifestValidator {
    ...
    pub fn validate_manifest(manifest: &Manifest) -> Result<(), Vec<ValidationError>> {
        ...
        if !is_valid_semver(&manifest.common.version) {
            errors.push(ValidationError {
                field: "version".into(),
                message: "version 必须为有效 semver".into(),
                code: "INVALID_SEMVER".into(),
            });
        }
        if let Some(repo) = &manifest.common.github_repo {
            if !repo.contains('/') || repo.split('/').count() != 2 {
                errors.push(ValidationError {
                    field: "githubRepo".into(),
                    message: "githubRepo 格式应为 owner/repo".into(),
                    code: "INVALID_FORMAT".into(),
                });
            }
        }
        match manifest.common.kind {
            Kind::Skill => {
                if manifest.skill.is_none() {
                    errors.push(ValidationError {
                        field: "skill".into(),
                        message: "kind 为 skill 时 skill.entry 必填".into(),
                        code: "REQUIRED".into(),
                    });
                } else if let Some(skill) = &manifest.skill {
                    if skill.entry.contains("..") {
                        errors.push(ValidationError {
                            field: "skill.entry".into(),
                            message: "不允许路径穿越（../）".into(),
            ...
            Kind::Mcp => {
                if manifest.mcp.is_none() {
                    errors.push(ValidationError {
                        field: "mcp".into(),
                        message: "kind 为 mcp 时 mcp.command 必填".into(),
                        code: "REQUIRED".into(),
                    });
                }
            ...
            Kind::Plugin => {
                if manifest.plugin.is_none() {
                    errors.push(ValidationError {
                        field: "plugin".into(),
                        message: "kind 为 plugin 时 plugin.entry 和 plugin.type 必填".into(),
                        code: "REQUIRED".into(),
                    });
                }
            }
...
mod tests {
    ...
    fn test_invalid_semver() {
        let json =
            r#"{"id":"t","name":"T","version":"abc","kind":"skill","skill":{"entry":"SKILL.md"}}"#;
        let errors = ManifestValidator::validate_json(json).unwrap_err();

...
/// **codex 权限切换相邻步骤间的硬性最短间隔**（毫秒；2026-09-23 用户指令）：
/// 步骤① `/permissions`+回车 与 步骤② 选数字 之间 **≥500ms**；有完全信任的
/// 步骤③（确认框数字）再隔 **≥500ms**。
///
/// **与轮询的关系（用户原话口径）=「并存，取最大」**：屏读轮询先 Ready 也要等满
/// 0.5s 才进下一步；0.5s 先到则轮询何时 Ready 何时触发。实现上=每步硬等 500ms
/// **后**才进入下一步的轮询窗（轮询窗预算不受此影响）——总间隔恒 ≥500ms，渲染

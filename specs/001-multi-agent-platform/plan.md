...
├── linker/              # 扩展资源映射（三层架构）
│   ├── mod.rs           # LinkerService（三层映射 + symlink/Junction/copy + LinkStatus 健康检查）
│   ├── layer1.rs        # SSOT 管理（统一仓库读写）
│   ├── layer2.rs        # 工具级激活目录管理
│   ├── layer3.rs        # 子Agent级激活目录管理（Hermes/OpenCode）
...
- 所有资源原始文件存放于此
- 更新资源时只需修改此处，Layer 2/3 通过 symlink 自动获得最新版本
- 备份/恢复时只需备份此目录
...
```
~/.mam/active/
├── claude/              # Claude Code 已启用的 skill
├── codex/               # Codex CLI 已启用的 skill
├── opencode/            # OpenCode 已启用的 skill
└── hermes/              # Hermes 已启用的 skill
```

- 每个工具一个目录，存放该工具已启用的 skill 链接
- 工具实际 skill 目录（如 `~/.claude/skills/`）symlink 到对应的 Layer 2 目录
- MCP 和插件直接写入工具配置文件，不经过此层
...
~/.mam/active/
├── hermes/
│   ├── researcher/      # Hermes researcher 子 Agent 的 skill
│   ├── coder/           # Hermes coder 子 Agent 的 skill
│   └── reviewer/        # Hermes reviewer 子 Agent 的 skill
└── opencode/
    ├── researcher/
    ├── coder/
    └── reviewer/
...
| 工具 | Layer 2 路径 | Layer 3 路径（如支持） | 实际工具目录 |
|------|-------------|----------------------|------------|
| Claude Code | `~/.mam/active/claude/` | 不支持 | `~/.claude/skills/` |
| Codex CLI | `~/.mam/active/codex/` | 不支持 | `~/.codex/skills/` |
| OpenCode | `~/.mam/active/opencode/` | `~/.mam/active/opencode/<agent>/` | `~/.config/opencode/skills/`（主）<br>`~/.config/opencode/agents/<agent>/skills/`（子） |
| Hermes | `~/.mam/active/hermes/` | `~/.mam/active/hermes/<agent>/` | `~/.hermes/skills/`（主）<br>`~/.hermes/agents/<agent>/skills/`（子） |
...
- 进程形态：进程名为 `hermes`
- 数据路径：`~/.hermes/sessions/`（JSONL 格式，与 Codex 类似）
- 子 Agent 标识：`payload.source.subagent` 字段
- 状态判断：与 Codex 类似
- 无 Hook 系统，依赖进程扫描 + 数据文件解析
...
一键应用预设组到目标工具/子Agent时：
1. **读取**预设组中的所有资源项（skill + MCP + 插件）
2. **检查**每项资源是否在全局仓库中存在（不存在则报错）
3. **检查**目标是否为子Agent级分配：若是，验证资源在工具级范围内
4. **启用**：对每项资源调用 `toggle_extension(extension_id, tool_id, subagent_id, enabled=true)`
   - Skill → 创建 symlink/Junction 到 Layer 2（工具级）或 Layer 3（子Agent级）
   - MCP → 按工具格式写入配置文件（JSON/TOML/JSONC）
   - Plugin → 文件型 symlink，配置型写入
5. **记录**应用状态到 `preset_applications` 表

### 取消激活策略

1. **读取**预设组中所有资源项
2. **禁用**：对每项资源调用 `toggle_extension(extension_id, tool_id, subagent_id, enabled=false)`
   - 移除 symlink/Junction（从 Layer 2/3）
   - 从配置文件中删除对应条目
...
- 取消激活预设组时，精确移除该预设组引入的 skill 链接；不存在"手动单独启用的 skill"，因此无冲突

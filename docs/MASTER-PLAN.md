...
| F1.6 | 系统级推送（可选） | **推送网关抽象**（同一事件源多出口）：Bark（iOS）/ ntfy（Android；**华为无 GMS 机型 Web Push 不可达，ntfy 为其默认出口**）；APK 出口（内置长连接，二期随 APK 交付）预留 |
...
| F1.10 | APK 加强包 **（2026-09-15 移出期，见 D15 修订）**：移至二期收尾——「手机端网页发送消息」落地后交付；Capacitor 壳 + 内置长连接推送 + 通知点击直达 |
...
- **一期**：外网手机扫码→看到八工具会话状态与内容；杀掉一个 agent 会话状态跃迁→手机 2s 内收到提醒；配对码刷新旧码失效；"停止远程"后所有设备 403；quick tunnel 断网自动恢复。（2026-09-15：APK 验收项随 D15 修订移至二期）

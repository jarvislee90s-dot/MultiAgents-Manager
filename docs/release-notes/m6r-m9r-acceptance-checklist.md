...
| F-2 | APP 软归档 | APP 会话（绿态）点归档钮 → 确认 | 卡片从手机看板消失；桌面端仍显示（桌面不动裁决）；历史页出现「未结束」徽标条目排最前 |
| F-3 | APP 非绿无钮 | APP 会话处理中（黄态） | 卡片不渲染归档钮 |
| F-4 | 自动回归 | 软归档的 APP 会话在桌面侧开始干活 | 手机看板自动重新出现（跃迁即时重拉或 ≤30s 对账），历史页条目消失 |

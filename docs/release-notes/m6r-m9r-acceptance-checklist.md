...
1. **实机项全部「登记未跑」**（红线允许的如实路径）：E1–E6 各 DoD 的 `#[ignore]` 实机
   用例未真机跑（需自 spawn 真实 CLI 会话+触发 busy/问答，超出单测编排职责）；键序均按
   词典清淤版定案实现且路由/守卫/解析锁在门禁内全绿。I-1 清单即补跑清单。

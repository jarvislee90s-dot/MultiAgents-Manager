...
| C-25 | **打断式插队（T9 新增；丁T6 复评改判据见 C-47）** | claude 交互会话处于**运行中**（busy） | 在手机端对运行中的 claude 会话点「立即发送」 | ① 当前回合被 Esc 中断（"Interrupted · What should Claude do instead?"）；② 消息**即刻进入**（而非停在 TUI 内部队列）；③ 审计 `action=jump`。**降级**：Esc 注入失败/等回合停超时仍继续投递正文（best-effort）。**实机探测已完成（2026-09-21）**：档案 `research/refs/phase2-消息注入/2026-09-21-claude打断式插队Esc语义链实机探测.md`——busy 注入正文 → Enter 落**内部队列**（底栏原文 `Press up to edit queued messages`，**即计划书问题 10 的现场证据**）；**Esc 中断后队列消息被自动取出开新回合**（`Interrupted…` → `✢ Photosynthesizing…`，**消息不丢**）→ 确证「先 Esc 再投递」次序正确。**丁T6 复评更正（2026-09-22 用户实机 bug）**：原第 ② 步判据是 `wait_input_drained`（等**我们自己的输入缓冲**排空）——Esc 写完缓冲随即就空，正文紧跟着落进**正在收尾的旧回合**，回执却报成功（**假成功**：审计 `jump ok` 但会话 JSONL 无消息正文）。已改为**屏读轮询等「回合已停」判据本身**（底栏 `esc to interrupt` 消失，D20 形态，3s 窗），超时不再冒充成功（回执降级为「已投递未确认」）——验收步骤见 C-47。**未测面（留白）**：`TURN_STOP_POLL_TOTAL_MS` 3s 预算的精确边界未逐毫秒量化（实测项 `d20_turn_stop_latency_live_probe` 已登记）；多条队列消息的取舍顺序未测；**「Esc 是否清输入行草稿」仍未直接验**；重复 Esc 时机未测 |

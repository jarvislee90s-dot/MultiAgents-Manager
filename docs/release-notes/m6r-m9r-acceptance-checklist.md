...
| C-24 | **模式切换（T6 新增；2026-09-23 codex 改造重写判据）** | 同上；四家各一在场 | ① 会话详情页「模式」栏显示当前档；② claude/opencode/kimi 点「切换模式」（shift+tab 循环切一档）；③ **codex（2026-09-23 改造后）**：点「计划 ⇄ 操作」单钮 toggle（= 向终端发一次 shift+tab，当前档未知时禁用），权限组四档「只读/默认/自动审批/完全信任」逐档按钮（数字直达；完全信任需前端二次确认）；④ **opencode 专项**：切后屏读回读档位应变化 | ① opencode 档位回读**变化可见**；② codex toggle 后底栏 `Plan mode` 出现/消失（回读验证）；权限档切换后见 `Permissions updated to <档>` 回执、verified=true 且权限组高亮+「（上次切换）」标注；完全信任走确认条（未确认零请求）；③ codex 运行中点 toggle → 「运行中不接受模式切换」。**实机证据链**：T6 探测（2026-09-21）四家 shift+tab 全生效 + opencode 回读 5/5；**codex 改造实测（2026-09-23 用户走查）**：shift+tab 计划/操作双向可切、菜单内数字 1/2/3 直选前三档、4→`Enable full access?` 二阶段按 1——键序词典按裁20 更新（见 I-3 第 7 条与台账）；**登记未跑**：菜单残留防护（esc 先行）、自动审批档全链、档位记忆显示（I-1k..m + I-2f） |
...
## F 段 · 看板关闭/归档 + 归档加载更早（体验批二，2026-09-20）

> 基准：feat/mobile-archive-history（体验批二提交起）。前置：MAM dev 运行（后端新端点生效需重启 tauri:dev）；手机刷新新构建的移动端产物（pnpm build:mobile）。

| # | 场景 | 步骤 | 预期观察 |
|---|---|---|---|
| F-1 | CLI 关闭终端 | CLI 会话卡片状态点左侧电源钮 → 确认 | 请求 ok 后卡片乐观消失；进程死（≤3s 扫描对齐）；审计 action=close result=ok；历史页出现该会话 |
| F-2 | APP 软归档（叉） | APP 会话点归档钮（**任意状态**，叉不挑颜色）→ 确认 | 卡片从手机看板消失且**不再自动回归**（含未读/状态变化）；未读池行被删（等同桌面端叉掉）；历史页出现「未结束」徽标条目排最前 |
| F-3 | APP 非绿可归档 | APP 会话处理中（黄态）点归档钮 → 确认 | 与绿态同行为：隐藏 + 历史页可移回 |
| F-4 | 无自动回归 | 软归档的 APP 会话在桌面侧开始干活/来新消息 | 手机看板**不**重新出现（叉掉=不再跟踪管理）；找回唯一入口 = 历史页「移回看板」 |
| F-5 | 移回看板 | 历史页「未结束」条目 → 详情页「移回看板」 | 回历史页；看板卡片回归；详情页无「在桌面端打开」（活会话不可 resume） |
| F-6 | 关闭误触保护 | 点开关后取消确认 | 不发请求，卡片保留 |
| F-7 | 归档加载更早 | 历史详情滚到顶（200 条）点「加载更早消息」 | 以翻倍 limit 整页重拉（200→400→…→1000 上限），视图锚定原顶部消息，到上限按钮消失 |
| F-8 | gate 回归 | 无凭据请求 session-close/hide/unhide | 403（nest 内结构性继承 PIN gate） |
...
| I-2f | codex 模式栏走查（2026-09-23 改造） | codex 会话详情页看模式栏 | ① 模式组=单钮「计划 ⇄ 操作」；② 权限组=**单钮「切换权限」**（2026-09-23 用户方案改造后，不再是四档逐档按钮）；③ 切换 verified 后权限组高亮+「（上次切换）」标注；④ 运行中点 toggle → 「运行中不接受模式切换」 |
| I-2g | **codex 权限「终端菜单单选题」全链（2026-09-23 用户方案）** | codex 会话详情页点「切换权限」 | ① 面板展开显示**终端菜单的档位**（编号徽标=屏上编号、文本=屏上档名，不含右侧描述列）；② 点某档 → 该档被选中（终端回执行可见），面板收起 + 回执带终端回执行原文；③ 切「完全信任」→ 面板切为终端**风险确认框**的两个选项 → 再点确认项才真正生效（**MAM 不代按**）；④ 屏上无菜单时点选 → 零投递 + 提示「请点重新读取」（不盲发数字键）；⑤ 「重新读取」（GET 零注入）能把面板与终端当前屏对齐；⑥ 关闭钮只收起面板（零请求，终端菜单仍开着——下次切换会自动 esc 清场） |
| I-2h | **屏读锚点账本（2026-09-23 建立）** | 升级 codex 后再试权限切换 | ① 工具改文案后，切换仍可用（账本持多组已知文案，认任意一句）；② 若某次仍读不到，**报错要点名**「在找哪些文案 + 本机版本」，便于一眼定位；③ 账本 append-only（旧文案不删——老版本工具仍在用） |
...
7. **codex 模式切换改造（2026-09-23，用户实机走查驱动，裁20「用户实测优先」）**：
   ① codex 模式组从 `/plan` 单向改为 **shift+tab 双向 toggle**（计划/操作），覆盖
   spec §2.6 旧「仅 /plan 单向、退出无实测命令」保守裁决；② codex 权限组从方向键
   闭环改为**数字直达**（菜单内数字键 1/2/3 直选、4→二阶段按 1，用户实测），覆盖
   旧「不盲发方向键」口径（「不盲发」精神保留：菜单锚窗+目标档恰一行才发数字键）；
   ③ 权限组三档扩**四档**（第 3 档「自动审批」=Approve for me，旧「不作目标档」
   裁决推翻）；④ 新增**菜单残留防护**（发 `/permissions` 前屏读见标题锚 → esc 先行
   ——用户走查事故「回车误确认当前项 + `/permissions` 输入行堆积」的根因修复）；
   ⑤ 新增**权限档「上次切换」记忆**（进程内存，重启清零；终端手改会失真——UI 标注
   如实声明口径）；⑥ codex 运行中拦截面从「模式组 Plan 档」扩为「模式组任意档」
   ...
   2026-09-23 追加批（屏读锚点账本 + 权限改终端菜单单选题，用户方案）登记：
   ① **事故根因**——codex 0.156.1 改菜单 footer 文案（`press enter to confirm or esc
   to go back` → `enter select · esc back`），而锚是写死单句 → 菜单窗不成立 → 权限切换
   整条静默失效（终端菜单明明开着）；② **裁决**——「版本最近邻取表」作为**排序规则**
   可用、作为**判据**不可用（把「我不知道」包装成「我知道」；版本号本身不可靠：codex
   npm 0.154.0 vs TUI 自报 0.155.1）；改用「屏幕证据是主键 + 账本认任意已知文案 +
   报错点名候选」；③ **用户方案**——权限档由用户从终端菜单点选（MAM 读回选项、只投递
   用户点的编号），消灭「后端猜编号」（编号随 Guardian 配置前移，硬编码 4→1 会错）；
   ④ **保留**——`POST /session-mode/switch` 旧自动路径不删（兜底，旧客户端仍可用），
   形成过渡期两条路（台账登记）；⑤ **已实机复验**：自建会话探针 dump 确认新 footer
   + 修复后内核读回四档（编号/高亮/档名全对，composer 判据不被菜单污染）；
   ⑥ **未跑**：I-2g 全链（面板交互）、I-2h 升级后复验——待用户手工走查。

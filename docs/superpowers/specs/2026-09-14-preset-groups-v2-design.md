...
- 日期：2026-09-14
- 状态：待用户审阅
- 前置：`specs/001-multi-agent-platform/spec.md` FR-6 / FR-24（预设组与批量分配）。本设计为其演进：FR-6.28~32 与 FR-24 主体语义兼容；**显式取代** FR-6 验收场景 2 的「再次点击该预设 = 整组禁用」toggle 语义（见 §5.5）。
...
- **常驻名单**：工具 × 资源，独占模式豁免；原生 MCP 段与工具自装插件**定义上即常驻**，无需标记。
...
对当前已激活的预设再次点击「应用」= **幂等重应用**：按该预设重新对齐（清掉会话期间漂移进来的额外资源、补齐缺失项），**不是**停用。「恢复默认」是独立的显式动作（预设卡片按钮 + 应用确认弹窗内入口）。本条**显式取代** specs/001 FR-6 验收场景 2「再次点击该预设 = 整组禁用」的 toggle 语义——独占模式 + 基底快照下，toggle 式一键还原有误触风险（想重新同步却整组还原），恢复统一走显式入口。
...
- **M1 后端语义**：schema 迁移 + DAO + 快照/清扫/暂存/恢复 + 专属绑定 + 数据源统一 + 全量 Rust 测试；AGENTS.md 数据目录表补 `~/.mam/stash/`。
...
| 2026-09-14 | 重复应用已激活预设 = 幂等重应用（非 toggle 停用），恢复默认走显式入口——显式取代 FR-6 验收场景 2（待用户最终确认） |

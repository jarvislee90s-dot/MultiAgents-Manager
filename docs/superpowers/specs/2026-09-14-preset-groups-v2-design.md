...
- **常驻名单**：工具 × 资源，独占模式豁免；原生 MCP 段与工具自装插件**定义上即常驻**，无需标记。**工具内建原生技能**（2026-09-16 裁决）同属此类：adapter 三层识别——① per-tool 静态清单（codex: `.system` / `_shared`，其余工具先空表）；② 目录内 `.codex-system-skills.marker` 标记（codex 自管重建标记，任意工具命中即内建）；③ 登记线兜底（未在 extensions 表登记 `is_native=1 AND source_tool` 行的磁盘原生目录视为常驻）。定义上即常驻、无需用户标记，**全路径保护**：不进快照、不被暂存、不可启停、不可卸载。**2026-09-18 裁决：常驻同时是停用防护**——常驻 on 时手动停用（skill/MCP/plugin）被拒，需先关闭常驻；启用方向不受影响；「全部禁用」批量跳过常驻项（前端配套）；W5 整工具停用的还原清理不受限。
...
| 2026-09-18 | 常驻同时是停用防护：常驻 on 时手动停用（skill/MCP/plugin）被拒，需先关闭常驻；启用方向不受影响；「全部禁用」批量跳过常驻项（前端配套）；W5 整工具停用的还原清理不受限 |

...
| 4 | 穿透：透明区点击透到下层；精灵/卡片/菜单可交互 | 已按 §16 预案降级 | **悬停穿透切换已按 §16 预案降级（窗口常驻交互、穿透禁用）**，原因 Tauri 2.11 无 `{forward:true}` 选项（JS/Rust API 均核实）：忽略态一旦生效事件流即断、无法悬停恢复，宠物将永久不可交互；故精灵/卡片/菜单常驻可交互，透明区点击不穿透（D11 风险项定性为已降级，见 §17.3 第 5 条） |
...
5. **D11 forward 缺口（已降级）**：Tauri 2.11 API 无 forward 选项，忽略态一旦生效事件流即断、无法悬停恢复（spec §4.4 风险命中），悬停穿透切换已按 §16 预案降级为窗口常驻交互（穿透禁用）；待 Tauri 提供 forward API 后恢复悬停切换。见 §17.2 第 4 项。

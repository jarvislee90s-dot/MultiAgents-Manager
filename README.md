...
手机浏览器打开与桌面**同源**的八工具会话看板：设置 → 远程接入 → 开启远程接入 → 绑定范围选「局域网 (0.0.0.0)」→ 手机扫二维码（或手动输入局域网地址）。当前为**临时直通模式**：一次性配对码换设备 cookie（180 天有效），刷新二维码即作废旧链接。
...
- **消息书签**：长会话里拖到某处觉得「待会回来看」，点书签条上的「+ 书签」选个颜色即可打标（最多 10 个，每色一个）；点色点跳回该处，点「管理」可删单个或清空全部。书签只在本次运行内保留（关掉 MAM 即清空），切回看板再进同一会话仍在
...
2. **切换绑定范围后必须「停止 → 开启」一次** —— 绑定变更不会热生效，切到局域网后若没有重启远程，服务器仍只挂在 127.0.0.1 上。验证方法：本机浏览器能打开「局域网地址」即绑定生效。
3. **手机地址必须带 `/m` 后缀** —— 完整地址形如 `http://192.168.x.x:9420/m`；推荐直接扫二维码（自带地址与配对码），手输漏掉 `/m` 会得到 404 空页。
4. **「访问地址」里的 `0.0.0.0` 不可拨号** —— 它表示「监听所有网卡」，不是能输进浏览器的地址；手机要用下方「局域网地址」。
